...
from PyQt6.QtWidgets import (
    ...
    QLabel, QMessageBox, QFileDialog, QStatusBar,
    QDialog, QTabWidget, QListWidgetItem,
    QSizePolicy, QGroupBox, QGraphicsDropShadowEffect, QScrollArea, QFrame,
    QPushButton, QLineEdit, QListWidget, QProgressBar, QCheckBox, QComboBox
)
from qfluentwidgets import (
    PushButton, PrimaryPushButton, LineEdit, PasswordLineEdit,
    ListWidget, CheckBox, ComboBox, ProgressBar, SimpleCardWidget,
    SearchLineEdit, setTheme, Theme, setThemeColor
)
            ...
            if results['next']:
                try:
                    ...
                    if e.http_status == 403 and "premium" in str(e).lower():
                        logger.warning(f"Spotify API limit reached (Premium required): stopping track fetch at {len(tracks)}")
                        break
                    else:
                        logger.error(f"Spotify API error fetching next page of tracks: {e}")
                        break
                except Exception as e:
...
class LoginWindow(QDialog):
    ...
    def init_ui(self):
        ...
        self.secret_input = PasswordLineEdit()
        self.secret_input.setPlaceholderText("Enter your Spotify Client Secret")

        secret_layout.addWidget(secret_label)
        secret_layout.addWidget(self.secret_input)

        self.show_secret_cb = CheckBox('Show Client Secret')
    ...
    def load_playlists(self):
        try:
            ...
            while results:
                ...
                if results['next']:
                    try:
                        ...
                        if e.http_status == 403 and "premium" in str(e).lower():
                            logger.warning(f"Spotify API limit reached (Premium required): loaded {len(self.playlists)} playlists")
                            QMessageBox.warning(
                                self,
                                'Spotify API Limitation',
                                'Your Spotify Developer App requires a Premium subscription to fetch more items.\n\n'
                                f'Only the first {len(self.playlists)} playlists could be loaded.'
                            )
                            break
                        else:
                            raise e
            ...
            if e.http_status == 403 and "premium" in str(e).lower():
                ...
                self.update_playlist_count()
                self.status_bar.showMessage(f'Loaded {len(self.playlists)} playlists (Limit reached)')
            else:
